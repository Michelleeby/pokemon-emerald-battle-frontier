...
static void (*const sDrawTextEntryBoxFuncs[])(void) =
{
    ...
    [NAMING_SCREEN_MOVE_SEARCH] = DrawNormalTextEntryBox,
};
...
{
    DoNamingScreen(NAMING_SCREEN_PLAYER, gSaveBlock2Ptr->playerName, GetPlayerAvatarStyle(), 0, 0, CB2_ReturnToFieldWithOpenMenu);
}

static void UNUSED Debug_NamingScreenBox(void)
{
    DoNamingScreen(NAMING_SCREEN_BOX, gSaveBlock2Ptr->playerName, GetPlayerAvatarStyle(), 0, 0, CB2_ReturnToFieldWithOpenMenu);
}

static void UNUSED Debug_NamingScreenCaughtMon(void)
{
    DoNamingScreen(NAMING_SCREEN_CAUGHT_MON, gSaveBlock2Ptr->playerName, GetPlayerAvatarStyle(), 0, 0, CB2_ReturnToFieldWithOpenMenu);
}

static void UNUSED Debug_NamingScreenNickname(void)
{
    DoNamingScreen(NAMING_SCREEN_NICKNAME, gSaveBlock2Ptr->playerName, GetPlayerAvatarStyle(), 0, 0, CB2_ReturnToFieldWithOpenMenu);
}
...
static const u8 sText_MoveName[] = _("MOVE NAME");

static const struct NamingScreenTemplate sMoveSearchNamingTemplate =
{
    .copyExistingString = FALSE,
    .maxChars = MOVE_NAME_LENGTH,
    .iconFunction = 2,
    .addGenderIcon = FALSE,
    .initialPage = KBPAGE_LETTERS_UPPER,
    .unused = 19,
    .title = sText_MoveName,
};
...
static const struct NamingScreenTemplate *const sNamingScreenTemplates[] =
{
    ...
    [NAMING_SCREEN_MOVE_SEARCH] = &sMoveSearchNamingTemplate,
};
