"""Prove that hard-mode Factory reaches Noland at the shortened streak."""
...
def _assert_completed_state(game: Session, addresses: dict[str, int]) -> None:
    ...
    if battle_num != 0:
        raise ScenarioFailure(
            "hard Noland route changed the ordinary Factory battle counter "
            f"to {battle_num}"
        )

    challenge_mode = game.read(addresses["challenge_mode"], width=8)
    if challenge_mode != FRONTIER_CHALLENGE_HARD:
        raise ScenarioFailure(
            f"Factory challenge mode is {challenge_mode}, expected hard"
        )

    win_streak = game.read(addresses["hard_win_streak"], width=16)
    if win_streak != HARD_NOLAND_COMPLETED_STREAK:
        raise ScenarioFailure(
            "hard Factory Singles Lv. 50 streak is "
            f"{win_streak}, expected {HARD_NOLAND_COMPLETED_STREAK}"
        )

    active_flags = game.read(addresses["hard_active_flags"], width=32)
    if not active_flags & STREAK_FACTORY_SINGLES_50:
        raise ScenarioFailure("hard Factory Singles Lv. 50 streak is not active")

    if game.read(addresses["normal_win_streak"], width=16) != 0:
        raise ScenarioFailure("hard Factory route changed the normal-mode streak")


def run(artifact_dir: Path) -> None:
    artifact_dir = Path(artifact_dir).resolve()
    save = artifact_dir / "factory-hard-noland.sav"
    ...
    create_factory_lobby_save(artifact_dir, save, noland_mode="hard")
    ...
    with Session(GAMEPLAY_ROM, artifact_dir / "scenario", save=save) as game:
        ...
        if (
            game.read(addresses["hard_win_streak"], width=16)
            != HARD_NOLAND_START_STREAK
        ):
            raise ScenarioFailure("hard Factory fixture did not start at streak 13")
